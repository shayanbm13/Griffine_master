...
class plug_frag : Fragment() {
    private  val SharedViewModel: SharedViewModel by activityViewModels()
    private val requestQueue: Queue<Pair<Plug, Button>> = LinkedList()
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_plug_frag, container, false)
    }
...
// پاک کردن صف و اضافه کردن آیتم باقی‌مانده از لیست
        ...
        isProcessing = true
        val (plug, button) = requestQueue.poll()
        val previousStatus = plug.status
        val plug_db = plug_db.getInstance(requireContext())

        Thread {
            try {
                val isSuccessful = udp_plug(requireContext(), plug)
                requireActivity().runOnUiThread {
                    if (isSuccessful) {
                        // Update button state based on new status
//                        plug.status = if (plug.status == "1") "0" else "1"
                        println("suseesssssssssss")
                        button.setBackgroundResource(if (plug_db.get_from_db_Plug(plug.id)?.status == "1") R.drawable.coler_on else R.drawable.coler_off)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var plug_layout =view.findViewById<ConstraintLayout>(R.id.plug_layout)
        val recyclerView: RecyclerView = view.findViewById(R.id.plug_recyclerview)
        ...
        SharedViewModel.current_room.observe(viewLifecycleOwner, androidx.lifecycle.Observer { room ->
            ...
            val learn_plug_db= plug_db.getInstance(requireContext())
//            learn_light_db.deleteRowsWithNullOrEmptyMac()
            SharedViewModel.update_plug_to_learn_list( learn_plug_db.getPlugsByRoomName(room!!.room_name))

            SharedViewModel.plug_to_learn_list.observe(viewLifecycleOwner, androidx.lifecycle.Observer { newlist ->
                val plug_status=view.findViewById<Button>(R.id.plug_status)
...
//                    Toast.makeText(requireContext(), selectedItem.Lname, Toast.LENGTH_SHORT).show()
                    ...
                    val set_timer_plug_item_menu=view.findViewById<ImageButton>(R.id.set_timer_plug_item_menu)
                    ...
                    Thread{
                        try {
                            ...
                            if (isConnectedToWifi(requireContext()) && ssid.replace("\"", "") ==db_ssid ){
                                ...
                                var is_ok=refresh_plug(this,selectedItem,room)
//                                println("sssssssssssssssss"+is_ok)

                                val handler = Handler(Looper.getMainLooper())
                                handler.post {
                                    if(is_ok){
                                        temp_disconnected.visibility=View.GONE
                                        plug_layout.alpha=0f
                                        plug_layout.visibility=View.VISIBLE
                                        plug_layout.animate().alpha(1f).setDuration(400).setListener(null)
                                    ...
                                    var status=database1.get_from_db_Plug(selectedItem.id)!!.status

                                    val alarmHandelerDb= alarm_handeler_db.getInstance(requireContext())
                                    val alarm_list=alarmHandelerDb.getAlarmsGroupedByDeviceName(selectedItem.Pname)
                                    ...
                                    fun on(){
                                        plug_status.setBackgroundResource(R.drawable.coler_on)
                                    ...
                                    fun off(){
                                        plug_status.setBackgroundResource(R.drawable.coler_off)
                                        status="0"
                                    }
                                    ...
                                    plug_status.setOnClickListener {
                                        println(status)
                                        try {
                                            requestQueue.add(Pair(selectedItem, plug_status))
                                            if (!isProcessing) {
                                                processQueue()
                                            }
//                            udp_curtain(this,selectedItem,"00")
...
// Minute Picker Off
                                    ...
                                    setalarm.setOnClickListener {
                                        ...
                                        if (alarm_list.count() < 4) {
                                            val groups=alarmHandelerDb.getAlarmsGroupedByDeviceName(selectedItem.Pname)
                                            if (days_list.count() > 0) {
                                                for (day in days_list) {
                                                    println(day)
                                                    val alarmName = "plug_" + selectedItem.Pname+"("+day+")" + "_1"
                                                    ...
                                                    alarm_on.device_name = selectedItem.Pname
                                                    ...
                                                    val alarmName2 = "plug_" + selectedItem.Pname+"("+day+")"+"_0" // نام یا شناسه یکتا برای الارم
                                                    ...
                                                    alarm_off.device_name = selectedItem.Pname
                                    ...
                                    yes_delete.setOnClickListener {
                                        ...
                                        if (alarm_list.count() > 0 ){
                                            ...
                                            for(alarmm in alarm_list[timetabel_list.indexOf(selected_timetabel[0])]){
                                                ...
                                                val alarmIntent = Intent(context, myBroadcastReceiverr::class.java)
                                                val pendingIntent = PendingIntent.getBroadcast(context, alarmName.hashCode(), alarmIntent,
                                                    PendingIntent.FLAG_CANCEL_CURRENT or PendingIntent.FLAG_IMMUTABLE)
                                                val alarmMgr = requireContext().getSystemService(Context.ALARM_SERVICE) as AlarmManager
