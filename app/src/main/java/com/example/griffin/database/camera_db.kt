package com.example.griffin.database
...
class camera_db private constructor(context: Context) : SQLiteOpenHelper(context, "camera_db", null, 1) {
    override fun onCreate(db: SQLiteDatabase?) {
        val CREATE_camera_db_TABLE =
            "CREATE TABLE IF NOT EXISTS " + "camera_db" + "(id INTEGER PRIMARY KEY,  user TEXT,  subtype TEXT ,  pass TEXT  ,  ip TEXT  ,  chanel TEXT,  CAMname TEXT,  port TEXT  )"
    ...
    fun get_from_db_camera(id: Int?): camera? {
        ...
        val cursor = db.query(
            "camera_db",
            arrayOf("id", "user", "subtype", "pass","ip","CAMname","port","chanel"),
            "id=?",
            arrayOf(id.toString()),
            null,
            null,
    ...
    fun getAllcameras(): List<camera?> {
        ...
        val cursor = db.query(
            "camera_db",
            arrayOf("id", "user", "subtype", "pass", "ip", "chanel", "CAMname", "port", "chanel"),
            null,
            null,
            null,
            null,
    ...
    fun getcamerasBychanelAddress(chanel: String?): List<camera?> {
        ...
        val cursor = db.query(
            "camera_db",
            arrayOf("id", "user", "subtype", "pass", "ip", "chanel", "CAMname", "port"),
            selection,
            selectionArgs,
            null,
            null,
