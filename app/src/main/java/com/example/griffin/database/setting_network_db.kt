...
class setting_network_db private constructor(context: Context) : SQLiteOpenHelper(context, "SETTING_NETWORK_DB", null, 1) {
    override fun onCreate(db: SQLiteDatabase?) {
        val CREATE_NETWORK_DB_TABLE = "CREATE TABLE IF NOT EXISTS " + "SETTING_NETWORK_DB" + "(id INTEGER PRIMARY KEY, modem_ssid TEXT , modem_password TEXT , api_key TEXT , city_name TEXT )"
...
//####  network database  #########################################################################################
    ...
    fun get_from_db_network_manual(id: Int): network_manual? {
        ...
        val cursor = db.query(
            "SETTING_NETWORK_DB",
            arrayOf("id", "modem_ssid", "modem_password", "api_key", "city_name"),
            "id=?",
            arrayOf(id.toString()),
            null,
            null,
