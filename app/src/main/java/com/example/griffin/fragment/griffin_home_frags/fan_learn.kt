...
import com.example.griffin.adapters.LearnfanAdadpter
...
class fan_learn : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_fan_learn, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val addfanitem_menu = view.findViewById<ImageButton>(R.id.addfan_menu)
        val recyclerView: RecyclerView = view.findViewById(R.id.fan_learn_recyclerview)
        ...
        SharedViewModel.current_room.observe(viewLifecycleOwner, Observer { room ->
            val database1= fan_db.getInstance(requireContext())
...
//            println(database.getThermostatsByRoomName(room!!.room_name))
            SharedViewModel.update_fan_to_learn_list( database.getfansByRoomName(room!!.room_name))
//            println(database.getThermostatsByRoomName(room!!.room_name))
            ...
            SharedViewModel.fan_to_learn_list.observe(viewLifecycleOwner, Observer { newlist ->
                ...
                val adapter = LearnfanAdadpter(newlist) { selectedItem ->
                    ...
                    val edit_fan_name_pupop = customPopupView.findViewById<EditText>(R.id.edit_light_name_pupop)
                    val ok_name_fan_pupop = customPopupView.findViewById<Button>(R.id.ok_name_light_pupop)
                    val learn_fan_pupop = customPopupView.findViewById<Button>(R.id.learn_light_pupop)
                    val delete_fan_pupop = customPopupView.findViewById<Button>(R.id.delete_light_pupop)
                    val on_off_test_fan_pupop = customPopupView.findViewById<Button>(R.id.on_off_test_light_pupop)
                    val send_to_pupop = customPopupView.findViewById<Button>(R.id.send_to)
                    on_off_test_fan_pupop.visibility=View.VISIBLE
                    send_to_pupop.visibility=View.VISIBLE
                    on_off_test_fan_pupop.isEnabled=false
                    ...
                    edit_fan_name_pupop.setText(selectedItem.Fname.toString())

//            edit_light_name_pupop.inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD
                    edit_fan_name_pupop.setOnClickListener{

                        edit_fan_name_pupop.selectAll()
                    }


                    learn_fan_pupop.setOnClickListener {
...
//                                                    .show()
                                                    ...
                                                    if (type == "Fano") {
                                                        ...
                                                        if (same_mac_to_learn != null) {
                                                            var counted=0
                                                            for (fan in same_mac_to_learn.subList(0,pole_num.toInt()) ){


                                                                val updated_fan= fan()
                                                                updated_fan.ip=fan!!.ip
                                                                updated_fan.mac=macip
                                                                updated_fan.room_name=selectedItem.room_name
                                                                updated_fan.status="0"
                                                                if (counted<9){

                                                                    updated_fan.subtype="000${counted+1}"
                                                                }else if (counted==9 ){
                                                                    updated_fan.subtype="0010"

                                                                }else if (counted>9 ){
                                                                    updated_fan.subtype="00${counted+1}"

                                                                }
                                                                updated_fan.Fname=fan.Fname
                                                                database.updatefanById(fan!!.id,updated_fan)
                                                                counted += 1
                                                            }
                                                            requireActivity().runOnUiThread{
                                                                ...
                                                                on_off_test_fan_pupop.alpha=1f
                                                                popupWindow.dismiss()
                                                            }
...
//                                                ).show()
                    ...
                    send_to_pupop.setOnClickListener {
                        ...
                        var adapter2 = changeroom_adapter(rooms_name){ newroomName ->
                            if (newroomName=="Favorite"){
                                ...
                                if (selectedItem.Fname !in favorite_names){

                                    var favorite_item= Favorite()
                                    favorite_item.type="fan"
                                    favorite_item.name=selectedItem.Fname
                                    ...
                                    popupWindow4.dismiss()
                                    Toast.makeText(requireContext(), "${selectedItem.Fname} exist in Favorite", Toast.LENGTH_SHORT).show()
                                ...
                                database.updatefanById(selectedItem.id,selectedItem)
                                popupWindow4.dismiss()
                                SharedViewModel.update_fan_to_learn_list( database.getfansByRoomName(room!!.room_name))
                            }
                    ...
                    val old_name = edit_fan_name_pupop.text.toString()
                    delete_fan_pupop.setOnClickListener {
                        ...
                        yes_delete.setOnClickListener {
                            ...
                            val selected_fan = database.get_from_db_fan(selectedItem.id)!!.Fname
                            val alarm_db= alarm_handeler_db.getInstance(requireContext())
                            try {
                                val alarms =alarm_db.getAlarmsWithSameDeviceName(selected_fan)
                                for (alarm in alarms){
...
// کنسل کردن الارم
                            ...
                            rename_item_in_scenario("d",old_name,old_name,"fan")
                            database.delete_from_db_fan(selectedItem.id)
//                        val alllight=database.getAllLights()
                            ...
                            SharedViewModel.update_fan_to_learn_list(database.getfansByRoomName(room!!.room_name))
                            if (database.getfansByRoomName(room!!.room_name).count() == 0){
                    ...
                    ok_name_fan_pupop.setOnClickListener {
                        if(old_name.trim() != edit_fan_name_pupop.text.toString().trim()){
                            ...
                            val same_counter = database.getNumberOfItemsByLname(edit_fan_name_pupop.text.toString().trim()).toInt()
                            if ( same_counter!= 0 ){
                                ...
                                try {
                                    ...
                                    alarm_db.updateNameById(alarm_db.get_from_db_alarm_by_deviceName(selected_fan)!!.id,edit_fan_name_pupop.text.toString().trim()+ " (${same_counter +1})")
                                }catch (e:Exception){
                                    println(e)
                                }


                                database.updateNameById(selectedItem.id,edit_fan_name_pupop.text.toString().trim()+ " (${same_counter +1})")
                            }else{
                                try {


                                    alarm_db.updateNameById(alarm_db.get_from_db_alarm_by_deviceName(selected_fan)!!.id,edit_fan_name_pupop.text.toString().trim())
                                }catch (e:Exception){
                                    println(e)
                                }


                                rename_item_in_scenario("e",old_name,edit_fan_name_pupop.text.toString().trim(),"fan")
                                database.updateNameById(selectedItem.id,edit_fan_name_pupop.text.toString().trim())
                            }


                            SharedViewModel.update_fan_to_learn_list(database.getAllfans())
                        }
        ...
        addfanitem_menu.setOnClickListener {
            val popupMenu = PopupMenu(requireContext(), addfanitem_menu)
            popupMenu.gravity = Gravity.TOP or Gravity.END
            popupMenu.menuInflater.inflate(R.menu.add_fan_menu, popupMenu.menu)
            ...
            popupMenu.setOnMenuItemClickListener { menuItem ->
                ...
                when (menuItem.itemId) {
                    R.id.add_fan -> {
                        val activity = requireActivity() as griffin_home
                        activity.viewPager.currentItem=16
// نمایش پاپ‌آپ
