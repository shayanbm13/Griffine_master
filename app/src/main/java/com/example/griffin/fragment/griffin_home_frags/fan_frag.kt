...
class fan_frag : Fragment() {
    private  val SharedViewModel: SharedViewModel by activityViewModels()


    private val requestQueue: Queue<Pair<fan, Button>> = LinkedList()
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_fan_frag, container, false)
    }
...
// پاک کردن صف و اضافه کردن آیتم باقی‌مانده از لیست
        ...
        isProcessing = true
        val (fan, button) = requestQueue.poll()
        val previousStatus = fan.status

        val fan_db = fan_db.getInstance(requireContext())
        Thread {
            try {
                val isSuccessful = udp_fan(requireContext(), fan)
                requireActivity().runOnUiThread {
                    if (isSuccessful) {
                        // Update button state based on new status
//                        fan.status = if (fan.status == "1") "0" else "1"
                        println("suseesssssssssss")
                        button.setBackgroundResource(if (fan_db.get_from_db_fan(fan.id)?.status == "1") R.drawable.coler_on else R.drawable.coler_off)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var fan_layout =view.findViewById<ConstraintLayout>(R.id.fan_layout)
        val recyclerView: RecyclerView = view.findViewById(R.id.fan_recyclerview)
        ...
        SharedViewModel.current_room.observe(viewLifecycleOwner, androidx.lifecycle.Observer { room ->
            ...
            val learn_fan_db= fan_db.getInstance(requireContext())
//            learn_light_db.deleteRowsWithNullOrEmptyMac()
            SharedViewModel.update_fan_to_learn_list( learn_fan_db.getfansByRoomName(room!!.room_name))

            SharedViewModel.fan_to_learn_list.observe(viewLifecycleOwner, androidx.lifecycle.Observer { newlist ->
                val fan_status=view.findViewById<Button>(R.id.fan_status)
...
//                    Toast.makeText(requireContext(), selectedItem.Lname, Toast.LENGTH_SHORT).show()
                    ...
                    val set_timer_fan_item_menu=view.findViewById<ImageButton>(R.id.set_timer_fan_item_menu)
                    ...
                    Thread{
                        ...
                        try {
                            ...
                            if (isConnectedToWifi(requireContext()) && ssid.replace("\"", "") ==db_ssid ){
                                ...
                                var is_ok=refresh_fan(this,selectedItem,room)
                                ...
                                handler.post {
                                    ...
                                    if (is_ok){
                                        ...
                                        fan_layout.alpha=0f
                                        fan_layout.visibility=View.VISIBLE
                                        fan_layout.animate().alpha(1f).setDuration(400).setListener(null)
                                    ...
                                    var status=database1.get_from_db_fan(selectedItem.id)!!.status

                                    val alarmHandelerDb= alarm_handeler_db.getInstance(requireContext())
                                    val alarm_list=alarmHandelerDb.getAlarmsGroupedByDeviceName(selectedItem.Fname)
                                    ...
                                    fun on() {
                                        fan_status.setBackgroundResource(R.drawable.coler_on)
                                    }

                                    fun off() {
                                        fan_status.setBackgroundResource(R.drawable.coler_off)
                                    }
                                    ...
                                    fan_status.setOnClickListener {
                                        try {
                                            requestQueue.add(Pair(selectedItem, fan_status))
                                            if (!isProcessing) {
                                                processQueue()
                                            }
                                        }catch (e:Exception){
...
// Minute Picker Off
                                    ...
                                    set_timer_fan_item_menu.setOnClickListener {
                                    ...
                                    setalarm.setOnClickListener {
                                        ...
                                        if (alarm_list.count() < 4) {
                                            ...
                                            val groups=alarmHandelerDb.getAlarmsGroupedByDeviceName(selectedItem.Fname)
                                            if (days_list.count() > 0) {


                                                for (day in days_list) {
                                                    println(day)
                                                    val alarmName = "fan_" + selectedItem.Fname+"("+day+")" + "_1"
                                                    ...
                                                    alarm_on.device_name = selectedItem.Fname
                                                    ...
                                                    val alarmName2 = "fan_" + selectedItem.Fname+"("+day+")"+"_0" // نام یا شناسه یکتا برای الارم
                                                    ...
                                                    alarm_off.device_name = selectedItem.Fname
